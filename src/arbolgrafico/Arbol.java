...
public class Arbol {
    ...
    public boolean agregar(int dato,String mensaje) {
        Nodo nuevo = new Nodo(dato, mensaje,null, null);
        insertar(nuevo, raiz);
        return true;
    }
    ...
    private Nodo borrar(Nodo r, int x) {
     ...
     private Nodo buscarNodo(Nodo r, int x) {
        if (r == null) {
            return (null);
        }
        int compara = ((Comparable) r.getDato()).compareTo(x);
        if (compara > 0) {
           r= buscarNodo(r.getIzq(), x);
        } else if (compara < 0) {
            r =buscarNodo(r.getDer(), x);
        } else {
            return r;
        }
        return r;
